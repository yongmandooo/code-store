...
class Cache(MemLike):
    ...
    def read(self, address):
        ...
        if self.is_hit(address):
            ...
            elif self.cacheline_size == 64:
                origin_addr = origin_addr << 6
            # origin_addr += offset #이거 왜그런지 모름

            origin_data = block.data
            lower_data = self.lower.read(address)
            if block.valid == 1:
                self.lower.write(origin_addr, origin_data)
            block.data = lower_data if lower_data != None and len(lower_data) == self.cacheline_size else [0] * self.cacheline_size
            block.valid = 1
            block.tag = tag
            ...
            return block.data[offset:offset + self.data_size]
